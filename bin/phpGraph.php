...
class phpGraph
{
	 ...
	 **/
	public function __construct($width = 600, $height = 300, $options = [])
	{
		if (!empty($options)) {
	 ...
	 * @author Cyril MAGUIRE
	 */
	public function draw($data, $options = [], $putInCache = false, $id = false, $txt = false)
	{
	...
	{
		$arrayOfMin = $arrayOfMax = $arrayOfLenght = $labels = $tmp['type'] = [];
		$valuesMax = $valuesMin = '';
		$Xmin = $Xmax = null;
		$multi = false;
		...
		return ['min' => $min, 'max' => $max, 'lenght' => $lenght, 'labels' => $labels, 'multi' => $multi, 'valuesMax' => $valuesMax, 'valuesMin' => $valuesMin, 'Xmin' => $Xmin, 'Xmax' => $Xmax, 'type' => $type];
	}
	...
	protected function __xAxisConfig($type, $width, $max, $Xmax, $Xmin, $lenght, $options = false)
	{
		...
		return ['XM' => $XM, 'stepX' => $stepX, 'steps' => $steps, 'unitX' => $unitX, 'widthViewBox' => $widthViewBox, 'Xmin' => $Xmin];
	}
	...
	protected function __header($dimensions = ['width' => '100%', 'height' => '100%', 'widthViewBox' => 685, 'heightViewBox' => 420], $responsive = true, $id = false)
	{
		if ($responsive == true) {
	 ...
	 *
	 * @param $data    array Array with structure equal to array('index'=>
	 *                 array('open'=>val,'close'=>val,'min'=>val,'max'=>val))
	 * @param $height  integer Height of grid
	 * @param $HEIGHT  integer Height of grid + title + padding top
	 * @param $stepX   integer Distance between two graduations on x-axis
	 * @param $unitY   integer Unit of y-axis
	 ...
	 * @author Cyril MAGUIRE
	 */
	protected function __drawDisk($data, $options = [], $id = false)
	{
		...
		if (isset($legends)) {
			if (!is_array($legends) && !empty($legends) && !is_bool($legends)) {
				...
				$legends = $Legends;
				unset($Legends);
			} else if (empty($legends)) {
				$notDisplayLegends = true;
			} else if (is_bool($legends)) {
				$legends = [];
			}
			foreach ($deg as $k => $v) {
	 ...
	 * @param $keys  mixed (array ou string) l'index ou le tableau des deux index sur le(s)quel(s) va(vont) se faire le
	 *               tri
	 *
	 * @return array
	 *
	...
	public function svg2vml($svg, $vml, $root, $xsl = 'vendors/svg2vml/svg2vml.xsl', $xslpath = '/svg2vml/')
	{
		if (is_string($svg)) {
			...
			$arguments = ['/_xml' => $xml_contents, '/_xsl' => $xsl];
	...
	public function wmodeTransparent($html)
	{
		...
		} else if (strpos($html, 'feature=oembed') !== false) {
